...
function draw_Arrow_STD(x,y, dx, dy, ctx, color='#000000') {
...
function draw_radial_field_ring(x, y, r, L, N, ctx, color){
    for (i = 0; i< 2*Math.PI; i+= 2*Math.PI/N) {
        draw_Arrow_STD(x + r*Math.cos(i), y + r*Math.sin(i), L*Math.cos(i), L*Math.sin(i), ctx, color);
    }
}